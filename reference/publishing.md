...
npx okona-cli create --title "Bumper Party" --description "Six cars, one arena." --runtime html5 [--genre party] [--controller rpg]
npx okona-cli controller <id> <basic|full|xl|rpg>   # the phone pad's layout (rpg = choices on the phone, Okona Pro); no republish
...
| `POST /games` | `{ title, description, runtime: "html5"\|"unity", genre?, controller? }` → `{ game }` (201) |
| `GET /games/{id}` · `PATCH /games/{id}` | read; update `title` / `description` / `genre` / `runtime` (patching a live game starts an update; the live build keeps serving) and/or `controller` — the phone pad's layout, `"rpg"` or `{ layout: "basic"\|"full"\|"xl"\|"rpg"\|null }`: a presentation field that applies to the next phone that connects and never starts an update; `rpg` is Okona Pro (403 otherwise). `game.controller.layout` reads it back |
