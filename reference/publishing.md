...
| `GET /games/{id}` · `PATCH /games/{id}` | read; update `title` / `description` / `genre` / `runtime` (patching a live game starts an update; the live build keeps serving) and/or `controller` — the phone pad's layout, `"full"` or `{ layout: "basic"\|"full"\|"xl" }`: a presentation field that applies to the next phone that connects and never starts an update. `game.controller.layout` reads it back (`basic` until the game picks)|`controller` — the phone pad's layout, `"full"` or `{ layout: "basic"\|"full"\|"xl" }`: a presentation field that applies to the next phone that connects and never starts an update. `game.controller.layout` reads it back (`basic` until the game picks)|`controller` — the phone pad's layout, `"full"` or `{ layout: "basic"\|"full"\|"xl" }`: a presentation field that applies to the next phone that connects and never starts an update. `game.controller.layout` reads it back (`basic` until the game picks)|`controller` — the phone pad's layout, `"full"` or `{ layout: "basic"\|"full"\|"xl" }`: a presentation field that applies to the next phone that connects and never starts an update. `game.controller.layout` reads it back (`basic` until the game picks)|`controller` — the phone pad's layout, `"full"` or `{ layout: "basic"\|"full"\|"xl" }`: a presentation field that applies to the next phone that connects and never starts an update. `game.controller.layout` reads it back (`basic` until the game picks) |
